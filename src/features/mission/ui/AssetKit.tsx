      ...
      <section className="space-y-3">
        ...
        <div className="bg-app-canvas border border-app-border rounded-xl p-4 space-y-3 shadow-sm transition-all duration-300">
          <div className="flex justify-between items-start">
            ...
            <div
              className={`px-1.5 py-0.5 rounded text-[8px] font-bold border ${settings.isDesanitized ? "bg-app-status-danger/10 border-app-status-danger text-app-status-danger" : "bg-app-status-ready/10 border-app-status-ready text-app-status-ready"}`}
            >
      ...
      {settings.isDesanitized && (
        <section className="space-y-4 animate-slideDown">
          ...
          <div className="bg-app-status-danger/5 border border-app-status-danger/20 rounded-xl p-4 space-y-3">
            <div>
              ...
              <p className="text-[9px] text-app-tertiary mt-1">
                Load framework source directly from your local repository clone.
              </p>
            </div>
            <div className="relative group">
              ...
              <button
                onClick={() =>
                  copyToClipboard("framework", frameworkLoaderSnippet)
                }
          ...
          <div className="bg-app-status-danger/5 border border-app-status-danger/20 rounded-xl p-4 space-y-3">
            <div>
              <p className="text-[10px] font-bold text-app-primary uppercase tracking-tighter">
                Mission Script Hot-Loader
              </p>
              <p className="text-[9px] text-app-tertiary mt-1">
                Iterate on your scripts without restarting missions. Provides UI
                feedback and logs to dcs.log.
              </p>
            </div>
            <div className="relative group">
              <pre className="p-2.5 bg-app-canvas border border-app-border rounded-lg text-[9px] font-mono text-app-tertiary overflow-x-auto select-all whitespace-pre-wrap">
              ...
              <button
                onClick={() =>
                  copyToClipboard("script", missionScriptLoaderSnippet)
                }
                className="absolute top-2 right-2 p-1.5 bg-app-surface border border-app-border rounded shadow-sm opacity-0 group-hover:opacity-100 transition-opacity hover:text-app-brand"
              >
      ...
      <section className="space-y-3">
        ...
        <div className="bg-app-canvas border border-app-border rounded-xl p-4 flex justify-between items-center shadow-sm">
          <div className="min-w-0">
            ...
            <p className="text-[9px] text-app-tertiary font-mono truncate">
              Source: GitHub csofranz/DML
            </p>
          </div>
          <a
        ...
        <div className="p-4 rounded-xl border border-app-border bg-app-surface/40">
          ...
          <div className="space-y-3">
            {[
              {
                step: "01",
                text: "Download and save Moose.lua to your mission folder.",
              },
              {
                step: "02",
                text: "In DCS ME, create a 'MISSION START' trigger.",
              },
              {
                step: "03",
                text: "Add Action: 'DO SCRIPT FILE' and select Moose.lua.",
              },
              {
                step: "04",
                text: "Load your generated mission scripts after Moose.",
              },
            ].map((item, i) => (
              <div key={i} className="flex gap-3 items-start">
                <span className="text-[9px] font-mono font-bold text-app-brand mt-0.5">
                  {item.step}
                </span>
                <span className="text-[10px] text-app-secondary leading-tight">
                  {item.text}
                </span>
              </div>
            ))}
