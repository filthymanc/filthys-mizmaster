...
const MOOSE_ROOT = globalThis.process.env.MOOSE_ROOT || path.resolve(PROJECT_ROOT, `../moose-${branch}/Moose Development/Moose`);

// Path to the output manifest file
const OUTPUT_PATH = globalThis.process.env.MANIFEST_OUTPUT_PATH || path.join(PROJECT_ROOT, `public/manifests/moose-${branch}.json`);
...
function getFrameworkVersion(mooseRoot, branchName) {
  try {
    ...
    if (fs.existsSync(gitDir)) {
      const version = execSync(`git -C "${path.dirname(gitDir)}" describe --tags --always`, { encoding: 'utf-8' }).trim();
      return version;
    }
  } catch {
    console.warn(`[Librarian] Could not detect git version for ${branchName}, falling back.`);
  }
  return branchName;
}
...
async function generateManifest() {
  ...
  const includeLines = content.match(/__Moose\.Include\(.*?'(.*?)'.*?\)/g) || [];
  ...
  for (const line of includeLines) {
    ...
    if (fs.existsSync(fullPath)) {
      ...
      while ((classMatch = classRegex.exec(fileContent)) !== null) {
        ...
        // Find Inheritance
        const ldocMatch = fileContent.match(new RegExp(`--\\s*@extends\\s+.*?#([A-Z_0-9]+)`, 'i'));
        const inheritMatch = fileContent.match(new RegExp(`BASE:Inherit\\s*\\(\\s*self\\s*,\\s*([A-Z_0-9]+):New`, 'i'));
        const parentName = ldocMatch ? ldocMatch[1] : (inheritMatch ? inheritMatch[1] : null);

        // Extract Description for this specific class
        let description = `MOOSE ${className} Class`;
        const descMatch1 = fileContent.match(new RegExp(`--- \\*\\*${className}\\*\\* ([^*]+)`, 'i'));
        const descMatch2 = fileContent.match(new RegExp(`--- \\*\\*(?:[^*]+)\\*\\* - ${className} ([^*]+)`, 'i'));
        ...
        const methods = {};
        const methodRegex = new RegExp(`function\\s+${className}[:\\.]([A-Za-z0-9_]+)\\s*\\(([^)]*)\\)`, 'g');
        let methodDef;
        ...
        manifest.classes[className] = {
          ...
          parent: (parentName === 'nil' || parentName === className) ? null : parentName,
          description,
          methods
        };
      }
