...
const DML_ROOT =
  globalThis.process.env.DML_ROOT || path.resolve(PROJECT_ROOT, "../DML");
const MODULES_DIR = path.join(DML_ROOT, "modules");
const OUTPUT_PATH =
  globalThis.process.env.MANIFEST_OUTPUT_PATH ||
  path.join(PROJECT_ROOT, "public/manifests/dml-main.json");
...
function getFrameworkVersion(dmlRoot) {
  try {
    ...
    if (fs.existsSync(gitDir)) {
      return execSync(`git -C "${dmlRoot}" describe --tags --always`, {
        encoding: "utf-8",
      }).trim();
    }
  } catch {
...
async function generateDmlManifest() {
  ...
  const files = fs
    ...
    .filter((f) => f.endsWith(".lua") && !f.includes("template"));
  ...
  for (const file of files) {
    ...
    const versionMatch = content.match(
      new RegExp(`${moduleName}\\.version\\s*=\\s*["']([^"']+)["']`),
    );
    if (versionMatch) {
    ...
    const attrRegex =
      /get(?:Bool|String|Number|PositiveRange)FromZoneProperty\s*\([^,]+,\s*["']([^"']+)["']/g;
    const noise = [
      "<none>",
      "none",
      "change",
    ...
    while ((attrMatch = attrRegex.exec(content)) !== null) {
      ...
      if (
        !noise.includes(attrName.toLowerCase()) &&
        !moduleInfo.attributes[attrName]
      ) {
        moduleInfo.attributes[attrName] = {
          name: attrName,
          type: attrName.endsWith("!")
            ? "trigger"
            : attrName.endsWith("?")
              ? "condition"
              : "property",
    ...
    while ((constMatch = constRegex.exec(content)) !== null) {
      ...
      if (
        !ignoreList.includes(constName) &&
        !constValue.startsWith("function") &&
        !constValue.startsWith("{")
      ) {
        if (!manifest.enums[moduleName]) {
          manifest.enums[moduleName] = {
            description: `Constants for ${moduleName}`,
            fields: [],
          };
        }
        if (
          !manifest.enums[moduleName].fields.find((f) => f.name === constName)
        ) {
  ...
  console.log(
    `✅ DML Manifest generated: ${OUTPUT_PATH} (${Object.keys(manifest.classes).length} modules found)`,
  );
}
